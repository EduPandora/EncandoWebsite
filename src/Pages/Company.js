...
const testimonials = [
    ...
    {
        id: 4,
        quote: <>Used the tool for a <span className="text-[var(--color-primary)] font-semibold">math heavy mechanics grad class</span> with 30+ students and no TA. I used it as a <span className="bg-[var(--color-primary)]/10 px-1 rounded font-semibold">unified platform</span> for everything from Discussion Boards to grading "mathy" homework. It cut my <span className="bg-[var(--color-primary)]/10 px-1 rounded font-semibold">grading time to about 1 hour</span> and allowed my students to get feedback within a day of submission (not just grades but also <span className="text-[var(--color-primary)] font-semibold">written feedback and solutions</span>). The discussion boards were also heavily used and <span className="text-[var(--color-primary)] font-semibold">cut my email traffic heavily</span> while being responsive to students. The autogenerated rubric was <span className="text-[var(--color-primary)] font-semibold">spot on</span> and the grading of handwritten homeworks was <span className="bg-[var(--color-primary)]/10 px-1 rounded font-semibold">unbelievably good</span>. Grading was consistent across all students and the interface was remarkably easy and intuitive. I am planning to use this for all my upcoming classes for sure.</>,
        name: "Arun Srinivasa",
...
const TestimonialCard = ({ testimonial }) => (
    <div 
        className="group relative bg-white rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-8 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-gray-100 flex flex-col h-full"
    >
